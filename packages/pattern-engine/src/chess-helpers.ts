...
export function isFiftyMoveRule(board: Chess) {
	const history = board.history({ verbose: true });
	if (history.length < 50) {
		return false;
	}

	const lastFiftyMoves = history.slice(Math.max(history.length - 50, 0));
	for (const move of lastFiftyMoves) {
		if (move.piece === PAWN || move.flags.includes(FLAG_CAPTURE)) {
			return false;
		}
	}
	return true;
